    ...
    tables = [
        {
            ...
            "col_count":  max(len(r) for r in t["rows"]),
            "heading":    t.get("heading", ""),
            "preview":    t["rows"][:2],
        }
        for i, t in enumerate(stitched, 1)
    ]
    # Pin Bilanz → 1, GuV → 2, Kapitalfluss → 3
    return _pin_key_tables(tables)
...
def _heuristic_word_extraction(pdf_path: Path) -> list[dict]:
    ...
    for page_num, page in enumerate(pdf.pages, 1):
        ...
        tables.append({
            ...
            "col_count":  max(len(r) for r in sorted_rows),
            "heading":    "",
            "preview":    sorted_rows[:2],
        })
