...
def _derive_audit_rows(raw_tables: list) -> list:
    ...
    """
    # Import here to keep the exporter usable independently while ensuring the
    # visible Mapping Audit uses the same framework/method guard as canonical
    # consolidation.
    from .consolidate import _load_exact_aliases, _map_actual

    aliases = _load_exact_aliases()
    out: list = []
    seen: set = set()
    for t in raw_tables:
        ...
        for row in (t.get("rows") or [])[1:]:
            ...
            mt = "none"
            if _HGB_AVAILABLE:
                record, mt, _ = _map_actual(
                    desc, aliases,
                    str(t.get("framework") or "unknown").lower(),
                    str(t.get("pnl_method") or "unknown").lower(),
                )
                if record:
                    std_id = record.get("std_id", "")
                    canon = record.get("canonical_en", "")
            out.append({
